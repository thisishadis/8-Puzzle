...
game.style.setProperty('--size' , array)
const puzzleSize = document.querySelector('.size').value
...
const puzzleSize = document.querySelector('.size').value
  ...
  buttons.forEach((button, index) => {
    button.textContent = array[index] === 0 ? "" : array[index];
    button.disabled = array[index] === 0;
  });
}
...
function isPuzzleSolved(puzzleSize) {
...
function startGame() {
  ...
  const numbers = Array.from({ puzzleSize }, (_, index) => index);
